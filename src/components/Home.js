...
class Home extends Component {
    ...
    constructor(props) {
        ...
        this.onHandlerInputChanges = this.onHandlerInputChanges.bind(this);
    ...
    /**
     * **componentWillMount**
     * @description es invocado _inmediatamente antes_ del inicio de el rendering
     */
    componentWillMount() {
    ...
    /**
     * **componentDidMount**
     * @description es invocado _inmediatamente despues_ del inicio de el rendering
     */
    componentDidMount() {
     ...
     * @description cuando el componente recive nuevas propiedades
     */
    componentWillReceiveProps( nexProps ){
        console.log('component Will Receive Props', nexProps);
    }
    
    /**
     * **shouldComponentUpdate**
     * @description antes del nuevo rendering,
     * pero despues de recivir las nuevas props o stado.
     * en esta funciom podemos retornar false para que el componente
     * no se dibuje
     */
    shouldComponentUpdate(nexProps, nexState){
        console.log('should Component Update', nexProps, nexState);
        //esto limita el cabio de links a 3
        // pero no lo hace se ve que no entendi nada
        if (nexState.status === 4 ) {
            return false;
        }
        return true;
    }
    ...
    componentWillUpdate(nexProps, nexState) {
        console.log('component Will Update', nexProps, nexState);
    }

    /**
     * **componentDidUpdate**
     * @description después de que las actualizaciones
     * de los componentes se desechan en DOM
     */
     ...
     * @description es invocado _inmediatamente antes_ de remover el componente del DOM
     */
    componentWillUnmount() {
    ...
    render() {
        return (
            <main className="App">
                ...
                <hr />
                {/**
                 * **utilizando una funcion recivida por prop, pero encapsulada en una funcion local**
                 * funcion en prop: _this.props.changeLink_
                 * funcion encapsuladora: _this.props.onChangeLinkName_
                 */}
                <input
                    type="text"
                    defaultValue={ this.state.homeLink }
                    onChange={ this.onHandlerInputChanges } />
                <button className="botonLoco" onClick={ this.onChangeLinkName }>Change Home Link!</button>
